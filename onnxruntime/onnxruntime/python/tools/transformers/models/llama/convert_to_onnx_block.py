...
from llama_inputs import get_merged_sample_with_past_kv_inputs, get_merged_sample_with_past_kv_inputs_mod, get_sample_inputs, get_sample_with_past_kv_inputs
...
def get_merged_model_dynamic_axes(input_names: list[str], output_names: list[str]):
    ...
    for name in input_names + output_names:
        if name in {"input_ids", "position_ids"}:
            # shape is (batch_size, sequence_length)
            dynamic_axes[name] = {1: "sequence_length"}
        elif name == "attention_mask":
            # shape is (batch_size, past_sequence_length + sequence_length) = (batch_size, total_sequence_length)
            # for prompt generation, past_sequence_length = 0
            # for token generation, sequence_length = 1
            dynamic_axes[name] = {1: "total_sequence_length"}
        elif "past" in name:
            # shape is (batch_size, num_heads, past_sequence_length, head_size)
            dynamic_axes[name] = {2: "past_sequence_length"}
        elif name == "logits":
            # shape is (batch_size, sequence_length, vocab_size)
            dynamic_axes[name] = {1: "sequence_length"}
        elif "present" in name:
            ...
            # for prompt generation, past_sequence_length = 0
            # for token generation, sequence_length = 1
            dynamic_axes[name] = {2: "total_sequence_length"}
        else:
            raise Exception("Unknown input or output name found")
    return dynamic_axes
...
def get_merged_model_dynamic_axes_mod(input_names: list[str], output_names: list[str]):
    dynamic_axes = {}
    for name in input_names + output_names:
...
def run_torchscript_merged_export(
    ...
    # Dummy values for export
    batch_size, sequence_length, past_sequence_length = 16, 8, 0
    ...
    # decoder_merged_inputs = get_merged_sample_with_past_kv_inputs_mod(
    #     l_config,
    #     device,
    #     batch_size,
    #     sequence_length,
    ...
    # dynamic_axes = get_merged_model_dynamic_axes_mod(input_names, output_names)
...
def main():
    ...
    # print(optimization_options.__dict__)
    # import pdb
    # pdb.set_trace()
